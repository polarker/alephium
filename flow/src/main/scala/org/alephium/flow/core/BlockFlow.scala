...
object BlockFlow extends StrictLogging {
  ...
  class BlockFlowImpl(
    ...
    def getBestIntraGroupTip(): BlockHash = {
      println(s"==== ${intraGroupHeaderChains
        .map(chain => chain.getAllTips.map(tip => tip.shortHex -> chain.getWeight(tip)))}")
      val bestTip = intraGroupHeaderChains.reduceBy(_.getBestTipUnsafe())(blockHashOrdering.max)
      println(s"==== ${bestTip.shortHex}")
      bestTip
    }
